...
class Cards:
    def get_spg_id(self):
        CardSkillsID = []
        reader = CsvReader()
        rowData = reader.readCsv(f'{reader.path}/csv_logic/cards.csv')
        for row in rowData:
    ...
    def get_unlock_by_brawler_id(self, brawler_id):
        reader = CsvReader()
        charsData  = reader.readCsv(f'{reader.path}/csv_logic/characters.csv')
        cardsData  = reader.readCsv(f'{reader.path}/csv_logic/cards.csv')
        for row in charsData:
            if charsData.index(row) == brawler_id:
                name = row[0]
                for row in cardsData:
